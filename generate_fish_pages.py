...
<head>
  ...
  <!-- Google tag (gtag.js) -->
  <script async src="https://www.googletagmanager.com/gtag/js?id=G-CFDC0LDR85"></script>
  <script>
    window.dataLayer = window.dataLayer || [];
    function gtag(){{dataLayer.push(arguments);}}
    gtag('js', new Date());
    gtag('config', 'G-CFDC0LDR85');
  </script>
  ...
  <script>
    Array.prototype.forEach.call(document.querySelectorAll('.store-link'), function (el) {{
      el.addEventListener('click', function (e) {{
        // GA4: クリック計測（魚種に関係なく一律 click_store 1件）
        if (typeof gtag === 'function') {{
          gtag('event', 'click_store');
        }}
        // iOS: itms-apps:// でアプリ起動を試みる
